...
(function () {
	...
	function ChartComponent($scope, $http, $timeout, $interval, authService, Constants) {
		var self = this;
		updateChartData();
			...
			$http.get(url).then(function (response) {
				...
				angular.forEach(response.data, function (queueStatus, date) {
					...
					if(queueStatus == null) {
						...
						self.chartData[4].push(null);
					} else {
						self.chartData[0].push(JSON.parse(queueStatus).BasketsQueues[0].size);
						if (JSON.parse(queueStatus).BasketsQueues[1]) {
							self.chartData[1].push(JSON.parse(queueStatus).BasketsQueues[1].size);
						}
						if (JSON.parse(queueStatus).BasketsQueues[2]) {
							self.chartData[2].push(JSON.parse(queueStatus).BasketsQueues[2].size);
						}
						//todo vladimir fix queues order
						if (JSON.parse(queueStatus).BasketsQueues[4] !== undefined) {
							self.chartData[3].push(JSON.parse(queueStatus).BasketsQueues[4].size);
							self.chartData[4].push(JSON.parse(queueStatus).BasketsQueues[3].size);
						} else {
							if (JSON.parse(queueStatus).BasketsQueues[3]) {
	...
	appModule.component('adChart', {
		templateUrl: 'app/shared/charts/chart.component.html',
